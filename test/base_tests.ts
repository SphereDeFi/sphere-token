...
describe("factory tests", function () {
  ...
  describe("remove tokens from multisig", async function () {
    before(async function () {
      ...
      await sphere.transfer(owner3.address, parseUnits("1000000"))
    ...
    it("removed balance of tokens from treasury", async function () {
      ...
      expect(await sphere.balanceOf(owner.address)).eq(parseUnits("4998000000"))
      ...
      expect(await sphere.balanceOf(owner3.address)).eq(parseUnits("1000000"))
    })
  })
})
