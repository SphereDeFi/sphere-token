...
contract BondDepo is Ownable {
  ...
  /**
   * @notice  returns the remaining payout as balanceOfSphere
   *          if bond is active, can't use moveBalance
   * @dev     Simsala
   * @param   who  Address to be checked
   * @return  uint256  balance of user
   */
  function balanceOfSphere(address who) public view returns (uint256) {
    uint256 payout = bondInfo[who].payout;
    uint256 pricePaid = bondInfo[who].pricePaid;
    if (payout == 0 || pricePaid == 0) {
      return 0;
    }
    return 1;
  }
